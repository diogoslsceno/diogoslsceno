...
class StatsIntegrityTests(unittest.TestCase):
    ...
    def test_workflow_falls_back_to_default_token_and_stages_only_generated_outputs(self):
        workflow = Path(".github/workflows/build.yaml").read_text(encoding="utf-8")

        # The workflow must always be able to run, even without the optional
        # ACCESS_TOKEN secret configured: it should fall back to the
        # Actions-provided github.token instead of failing the run outright.
        # Private-repository stats are simply omitted in that case.
        self.assertIn(
            "ACCESS_TOKEN: ${{ secrets.ACCESS_TOKEN || github.token }}", workflow
        )
